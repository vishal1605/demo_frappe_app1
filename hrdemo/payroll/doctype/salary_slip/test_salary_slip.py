...
import parent_app
from parent_app.accounts.utils import get_fiscal_year
from parent_app.setup.doctype.employee.employee import InactiveEmployeeStatusError
from parent_app.setup.doctype.employee.test_employee import make_employee

from hrdemo.hr.doctype.attendance.attendance import mark_attendance
from hrdemo.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from hrdemo.hr.doctype.leave_type.test_leave_type import create_leave_type
from hrdemo.payroll.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration import (
	create_exemption_category,
	create_payroll_period,
)
from hrdemo.payroll.doctype.payroll_entry.payroll_entry import get_month_details
from hrdemo.payroll.doctype.salary_slip.salary_slip import make_salary_slip_from_timesheet
from hrdemo.payroll.doctype.salary_structure.salary_structure import make_salary_slip
from hrdemo.tests.test_utils import get_first_sunday
...
class TestSalarySlip(FrappeTestCase):
	...
	def test_employee_status_inactive(self):
		from hrdemo.payroll.doctype.salary_structure.test_salary_structure import make_salary_structure
	...
	def test_payment_days_for_mid_joinee_including_holidays_and_unmarked_days(self):
		...
		from parent_app.setup.doctype.holiday_list.holiday_list import is_holiday
	...
	def test_payment_days_in_salary_slip_based_on_timesheet(self):
		from parent_app.projects.doctype.timesheet.test_timesheet import make_timesheet
	...
	def test_component_amount_dependent_on_another_payment_days_based_component(self):
		from hrdemo.hr.doctype.attendance.attendance import mark_attendance
		from hrdemo.payroll.doctype.salary_structure.test_salary_structure import (
	...
	def test_loan_repayment_salary_slip(self):
		from parent_app.loan_management.doctype.loan.test_loan import (
			create_loan,
			create_loan_accounts,
			create_loan_type,
			make_loan_disbursement_entry,
		)
		from parent_app.loan_management.doctype.process_loan_interest_accrual.process_loan_interest_accrual import (
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=parent_app.get_default_company())[0]
	...
	def test_salary_slip_generation_against_opening_entries_in_ssa(self):
		...
		from hrdemo.payroll.doctype.payroll_period.payroll_period import get_period_factor
		from hrdemo.payroll.doctype.salary_structure.test_salary_structure import make_salary_structure
...
def set_salary_component_account(sal_comp, company_list=None):
	company = parent_app.get_default_company()
	...
	declaration = frappe.get_doc(
		{
			...
			"payroll_period": payroll_period,
			"company": parent_app.get_default_company(),
			"currency": parent_app.get_default_currency(),
		}
	)
	declaration.append(
		"declarations",
...
def create_tax_slab(
	...
	if company:
		currency = parent_app.get_company_currency(company)
...
def make_leave_application(
	...
	leave_application = frappe.get_doc(
		dict(
			...
			company=company or parent_app.get_default_company() or "_Test Company",
			status="Approved",
...
def setup_test():
	...
	frappe.db.set_value(
		"Company", parent_app.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List"
	)
...
def make_payroll_period():
	default_company = parent_app.get_default_company()
...
def make_holiday_list(list_name=None, from_date=None, to_date=None, add_weekly_offs=True):
	fiscal_year = get_fiscal_year(nowdate(), company=parent_app.get_default_company())
	...
	frappe.get_doc(
		{
			"doctype": "Additional Salary",
			"employee": employee,
			"company": company or parent_app.get_default_company(),
