...
frappe.ui.form.on('Gratuity', {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 1 && !frm.doc.pay_via_salary_slip && frm.doc.status == "Unpaid") {
			frm.add_custom_button(__("Create Payment Entry"), function () {
				return frappe.call({
					method: "hrdemo.overrides.employee_payment_entry.get_payment_entry_for_employee",
					args: {
						"dt": frm.doc.doctype,
						"dn": frm.doc.name
					},
	...
	calculate_work_experience_and_amount: function (frm) {
		...
		if (frm.doc.employee && frm.doc.gratuity_rule) {
			frappe.call({
				method: "hrdemo.payroll.doctype.gratuity.gratuity.calculate_work_experience_and_amount",
				args: {
