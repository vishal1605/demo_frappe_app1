...
frappe.provide("hrdemo.hr");
frappe.provide("parent_app.accounts.dimensions");

frappe.ui.form.on('Expense Claim', {
	onload: function(frm) {
		parent_app.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);
	},
	company: function(frm) {
		parent_app.accounts.dimensions.update_dimension(frm, frm.doctype);
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			frappe.call({
				method: "hrdemo.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
				args: {
...
frappe.ui.form.on("Expense Claim", {
	setup: function(frm) {
		...
		frm.set_query("expense_approver", function() {
			return {
				query: "hrdemo.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
					employee: frm.doc.employee,
		...
		frm.set_query("employee", function() {
			return {
				query: "parent_app.controllers.queries.employee_query"
			};
		});
	},
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "hrdemo.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	make_payment_entry: function(frm) {
		let method = "hrdemo.overrides.employee_payment_entry.get_payment_entry_for_employee";
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "hrdemo.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		parent_app.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		...
		if (frm.doc.employee) {
			return frappe.call({
				method: "hrdemo.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
...
frappe.ui.form.on("Expense Claim Detail", {
	...
	cost_center: function(frm, cdt, cdn) {
		parent_app.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	}
});
