...
frappe.ui.form.on('Employee Onboarding', {
		...
		if ((!frm.doc.employee) && (frm.doc.docstatus === 1)) {
			frm.add_custom_button(__('Employee'), function () {
				frappe.model.open_mapped_doc({
					method: "hrdemo.hr.doctype.employee_onboarding.employee_onboarding.make_employee",
					frm: frm
				});
			}, __('Create'));
	...
	employee_onboarding_template: function(frm) {
		...
		if (frm.doc.employee_onboarding_template) {
			frappe.call({
				method: "hrdemo.controllers.employee_boarding_controller.get_onboarding_details",
				args: {
