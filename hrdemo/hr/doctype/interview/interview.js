...
frappe.ui.form.on('Interview', {
	onload: function (frm) {
		...
		frm.set_query('interviewer', 'interview_details', function () {
			return {
				query: 'hrdemo.hr.doctype.interview.interview.get_interviewer_list'
			};
		});
	},
				...
				frappe.db.get_value('Interview Feedback', {'interviewer': frappe.session.user, 'interview': frm.doc.name, 'docstatus': 1}, 'name', (r) => {
					if (Object.keys(r).length === 0) {
						frm.add_custom_button(__('Submit Feedback'), function () {
							frappe.call({
								method: 'hrdemo.hr.doctype.interview.interview.get_expected_skill_set',
								args: {
	...
	show_feedback_dialog: function (frm, data) {
		...
		let d = new frappe.ui.Dialog({
			...
			primary_action: function(values) {
				frappe.call({
					method: 'hrdemo.hr.doctype.interview.interview.create_interview_feedback',
					args: {
						data: values,
	...
	set_interview_details: function (frm) {
		frappe.call({
			method: 'hrdemo.hr.doctype.interview.interview.get_interviewers',
			args: {
				interview_round: frm.doc.interview_round
			},
