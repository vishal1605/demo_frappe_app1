...
frappe.ui.form.on('Employee Loan', {
	...
	make_jv: function (frm) {
		frappe.call({
			...
			method: "hrdemo.hr.doctype.employee_loan.employee_loan.make_jv_entry",
			callback: function (r) {
				if (r.message)
	...
	mode_of_payment: function (frm) {
		if (frm.doc.mode_of_payment && frm.doc.company) {
			frappe.call({
				method: "parent_app.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
				args: {
	...
	employee_loan_application: function (frm) {
	    if(frm.doc.employee_loan_application){
            return frappe.call({
                method: "hrdemo.hr.doctype.employee_loan.employee_loan.get_employee_loan_application",
                args: {
