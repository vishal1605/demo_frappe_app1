...
frappe.treeview_settings["Goal"] = {
	get_tree_nodes: "hrdemo.hr.doctype.goal.goal.get_children",
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
			options: parent_app.utils.get_tree_options("company"),
			label: __("Company"),
			default: parent_app.utils.get_tree_default("company")
		},
		{
	...
	fields: [
		...
		},
		{
			...
			get_query() {
				return {
					query: "hrdemo.hr.doctype.appraisal.appraisal.get_kras_for_employee",
					filters: {
	...
	get_tree_root: false,
	add_tree_node: "hrdemo.hr.doctype.goal.goal.add_tree_node",
			...
			click: function(node) {
				const dialog = new frappe.ui.Dialog({
					...
					primary_action: function() {
						dialog.hide();
						return frappe.call({
							method: "hrdemo.hr.doctype.goal.goal.update_progress",
							args: {
