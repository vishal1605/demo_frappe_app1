...
frappe.provide("parent_app.job_offer");
...
frappe.ui.form.on("Job Offer", {
	...
	select_terms: function (frm) {
		parent_app.utils.get_terms(frm.doc.select_terms, frm.doc, function (r) {
			if (!r.exc) {
	...
	refresh: function (frm) {
		if ((!frm.doc.__islocal) && (frm.doc.status == 'Accepted')
			...
			frm.add_custom_button(__('Create Employee'),
				function () {
					parent_app.job_offer.make_employee(frm);
				}
			);
		}
...
parent_app.job_offer.make_employee = function (frm) {
	frappe.model.open_mapped_doc({
		method: "hrdemo.hr.doctype.job_offer.job_offer.make_employee",
		frm: frm
	});
};
