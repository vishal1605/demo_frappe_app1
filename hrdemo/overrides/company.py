...
from parent_app.accounts.doctype.account.account import get_account_currency
...
def delete_company_fixtures():
	...
	for country in countries:
		try:
			module_name = f"hrdemo.regional.{frappe.scrub(country)}.setup.uninstall"
			frappe.get_attr(module_name)()
		except (ImportError, AttributeError):
			# regional file or method does not exist
			pass
		except Exception:
			frappe.log_error("Unable to delete country fixtures for hrdemo")
			frappe.throw(
...
def run_regional_setup(country):
	try:
		module_name = f"hrdemo.regional.{frappe.scrub(country)}.setup.setup"
		frappe.get_attr(module_name)()
	except ImportError:
		pass
	except Exception:
		frappe.log_error("Unable to setup country fixtures for hrdemo")
		frappe.throw(
...
def make_salary_components(country):
	...
	if not frappe.db.exists("Salary Component", "Basic"):
		file_path = frappe.get_app_path("hrdemo", "payroll", "data", file_name)
		docs.extend(json.loads(read_data_file(file_path)))

	file_path = frappe.get_app_path("hrdemo", "regional", frappe.scrub(country), "data", file_name)
	docs.extend(json.loads(read_data_file(file_path)))
